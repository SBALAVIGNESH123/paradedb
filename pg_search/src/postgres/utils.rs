    ...
    ) in categorized_fields
    {
        ...
        if *is_array {
            let converted_array = match search_field.field_type() {
                ...
                // Legacy pre-v0.24.0 indexes stored TIMESTAMP AND TIMESTAMP WITH TIMEZONE as DATE in the tantivy schema.
                SearchFieldType::Date(oid) if oid == pg_sys::TIMESTAMPOID => {
                    TantivyValue::try_from_timestamp_array_date(actual_datum)
                }
                SearchFieldType::Date(oid) if oid == pg_sys::TIMESTAMPTZOID => {
                    TantivyValue::try_from_timestamptz_array_date(actual_datum)
                }
